<script>
  ...
  $effect(() => { api(`/matches/${id}`).then((m) => (match = m)).catch((e) => (error = e.message)); });
  async function remove() {
    if (!confirm(`Spiel gegen ${match.opponent} mit allen Aktionen löschen?`)) return;
    try { await api(`/matches/${id}`, { method: 'DELETE' }); showToast('Gelöscht'); goto('/spiele'); }
...
<main class="page">
  <div class="toolbar"><h1>Spiel bearbeiten</h1><span class="spacer"></span>
    {#if match}<a class="btn" href="/auswertung/{id}">Auswertung</a>{/if}
  ...
  {:else if match}
    {#if match.state?.set > 1 || match.actions?.length}
      <p class="small muted" style="margin:0 0 12px">Aufstellung für Satz
        {#each Array.from({ length: Math.max(match.state.set, 1) }, (_, i) => i + 1) as s}<a href="/spiele/{id}?set={s}" style="margin-left:6px; font-weight:{s === set ? 700 : 400}">{s}</a>{/each}
